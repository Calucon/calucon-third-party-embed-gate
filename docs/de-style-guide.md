...
| „…switching it to enforced" / „…an enforced policy" (the two Content-Security-Policy helper strings) | **„erzwingen"/„erzwungen"**, not „durchsetzen". No glossary row decides this: the CSV has no `enforce`, `force`, `apply` or `policy` row at all, so neither word can be justified from it. The argument is the sense of the German — „durchsetzen" is overcoming resistance, while a browser leaving report-only mode is simply applying the policy rather than only logging what it would have blocked. Which form to use depends on the slot: the *name* of the mode is the quoted infinitive („sie auf „erzwingen“ umzustellen"), the *attribute* of the policy is the participle („eine erzwungene Richtlinie"). Approved on translate.wordpress.org. | 2026-09-01 |
