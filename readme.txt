...
* Security: the HTML scanner reads markup the way a browser does — a tag opens only where a browser opens one. Before, a comment opener or a raw-text tag name inside an attribute value (`<div data-x="<!--">`) hid every embed after it from the gate, silently, and an `<iframe` inside an `alt` text got a placeholder spliced into the attribute; both are within reach of any author, since WordPress keeps `<` inside attribute values.
