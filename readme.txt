...
* Fixed: on WordPress older than 6.7, the bundled German never reached the block editor's own controls — the front end and the settings screen were translated, the editor was not. `wp_set_script_translations()` was not being told where the plugin keeps its translation files, so WordPress looked only in the language-pack directory.
