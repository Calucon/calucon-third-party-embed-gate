...
if ! wpcli core is-installed >/dev/null 2>&1; then
  ...
  wpcli core install \
    --url="${URL}" \
    --title="Third-Party Embed Gate Test Site" \
    --admin_user=admin \
...
wpcli plugin activate third-party-embed-gate
wpcli eval-file /var/www/html/wp-content/plugins/third-party-embed-gate/tests/wp/seed.php
