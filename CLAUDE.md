...
**Deliberately NOT renamed** (stored data and markup contract — do not
"finish" the rename into these):
- The `cg-`/`data-cg-`/`--cg-` markup prefix (§5.1 contract; read it as
  "Calucon Gate"). Keeping it also keeps the whole fixture corpus
  byte-identical — proof the rename never changed output.
- The block **attribute keys** `consentGate`/`consentGatePoster`/
  `consentGatePosterUrl` — stored in published post content (block JSON).
- **Back-compat aliases**, each marked "remove no earlier than 1.0.0, in a
  release of its own": `CONSENT_GATE_*` constant aliases (the calucon.de
  child theme reads the version constant to stand down its own gate —
  detection failure = duplicate panels), `[consent_gate_withdraw]`
  shortcode, `consent-gate/withdraw` block, `wp consent-gate` CLI,
  `{theme}/consent-gate/` template fallback, the `consent_gate_options` →
  new-name option migration in `Plugin::__construct`, the localStorage key
  migration in gate.js, and uninstall.php deleting both option names.
