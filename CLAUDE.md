...
`consent_gate_*` hooks/option/shortcode, `cg-`/`consent-gate` CSS classes
and enqueue handles, `wp consent-gate` CLI, the `{theme}/consent-gate/`
template override path. Never "finish" the rename into those internals —
the option name and hook names are live site data and public API. One
exception, made in 0.8.1 because it has a real external consumer: the
bootstrap constants are now `CALUCON_EMBED_GATE_{VERSION,FILE,DIR}` — the
child theme on calucon.de reads the version constant to stand down its own
embed gate (both gates running = duplicate panels on every embed). The old
`CONSENT_GATE_*` names stay defined as aliases until at least 0.9.0 and
must be removed only in a release of their own. The i18n bridge strings ($t()) are
mirrored as literal `__()` calls in the generated `languages/strings.php`
(regenerated by `generate-pot.php`) so translate.wordpress.org can extract
them; keep it in the build-zip whitelist.
